...
class TeltonikaConfiguration extends Model
{
    ...
    public function plcAnalyticsGraphs()
    {
        ...
        return Graph::where('machine_id', $machine_id)->where('graph_id', '<', 100)->where('graph_name', '!=', "Energy Consumption")->get();
    }
    ...
    public function plcEnabledAnalyticsGraphs($user_id, $serial_number)
    {
        ...
        if ($v) {
            return Graph::where('machine_id', $machine_id)
                ->where('graph_name', '!=', "Energy Consumption")
                ->whereIn('graph_id', json_decode($v->property_ids))->where('graph_id', '<', 100)->get();
        } else
            return Graph::where('machine_id', $machine_id)
                ->where('graph_name', '!=', "Energy Consumption")
                ->where('graph_id', '<', 100)->get();
    }
